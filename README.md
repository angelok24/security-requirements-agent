...
4. **Уточнение компонентов и потоков (human-in-the-loop).** Агент выводит распознанную таблицу потоков данных, и специалист может её поправить — исправить неверно определённые компоненты и добавить пропущенные связи. После этого архитектурные признаки (наличие БД, IAM, внешнего ИИ-сервиса и т.д.) пересчитываются, и список требований формируется заново — уже точнее.
5. **Генерация отчёта (LLM).** Формируется аналитический отчёт: резюме системы, ключевые риски, приоритеты выполнения.
6. **Оценка качества.** Результат сравнивается с эталонными требованиями эксперта по метрикам **Precision / Recall / F1** (сопоставление через косинусное сходство TF-IDF).
...
**5. Уточнение компонентов (human-in-the-loop).** После ручной правки таблицы потоков архитектурные флаги пересчитываются по ключевым словам, и требования формируются заново — уже с учётом исправленной архитектуры:

```python
arch['data_flows'] = flows_manual          # исправленная специалистом таблица потоков
all_text = ' '.join(f['from'] + ' ' + f['to'] + ' ' + f['data'] for f in flows_manual).lower()
...
# ... остальные флаги ...
reqs = step2_map_requirements(arch, CRITICALITY, HAS_PDN)   # пересчёт требований
```

**6. Авторизация GigaChat.** В отличие от Яндекса (ключ сразу в заголовке), GigaChat сначала обменивает ключ на временный токен доступа и обновляет его при истечении:
...
**7. Оценка качества.** Требования агента и эксперта переводятся в TF-IDF-векторы; по косинусному сходству ищутся совпадения (порог 0.35), из которых считаются Precision / Recall / F1:
